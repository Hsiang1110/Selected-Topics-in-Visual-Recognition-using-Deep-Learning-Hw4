...
if __name__ == '__main__':
    ...
    train_dataset = DatasetFromFolder(f"{args.dataroot}",
                                    image_size=args.image_size,
                                    scale_factor=args.scale_factor)
    val_dataset = DatasetFromFolder(f"{args.dataroot}",
                                    image_size=args.image_size,
                                    scale_factor=args.scale_factor)
    print('len: ', len(train_dataset))

    train_size = int(0.9 * len(val_dataset))
    valid_size = len(val_dataset) - train_size
    _, valid_dataset = torch.utils.data.random_split(val_dataset, [train_size, valid_size])
